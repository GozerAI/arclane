...
if [ ! -d "$REPO_DIR" ]; then
    mkdir -p "$APP_DIR"
    git clone https://github.com/GozerAI/arclane.git "$REPO_DIR"
else
...
if [ ! -f "$REPO_DIR/.env" ]; then
    ...
    sed -i "s/change-me-in-production/$SECRET/" "$REPO_DIR/.env"
    # Set production defaults
    sed -i 's|ARCLANE_DATABASE_URL=.*|ARCLANE_DATABASE_URL=sqlite+aiosqlite:///data/arclane.db|' "$REPO_DIR/.env"
    echo ""
