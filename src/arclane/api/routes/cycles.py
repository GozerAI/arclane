...
from arclane.services.roadmap_service import _week_group_for_day, get_phase_tasks
...
async def trigger_cycle(
    ...
    # Check if this cycle would produce any work before deducting a working day.
    # For roadmap-driven cycles: if no task is defined for today's day AND
    # no custom task description was provided, skip the working day deduction.
    # The cycle still runs (and auto-advances the phase if graduation is met).
    is_roadmap_phase = (
        not payload.task_description
        and 1 <= (business.current_phase or 0) <= 4
        and not orchestrator.next_queue_task(business)  # no operating-plan task queued
    )
    skip_working_day = False
    if is_roadmap_phase:
        week_group = _week_group_for_day(business.current_phase or 0, business.roadmap_day or 0)
        if not week_group or not get_phase_tasks(business.current_phase or 0, week_group):
            skip_working_day = True

    if not used_included_cycle and not skip_working_day:
        # Atomic working day deduction - deduct bonus first, then regular.
        # Uses SQL-level decrement with a WHERE guard to prevent race conditions.
        result = await session.execute(
            update(Business)
            .where(Business.id == business.id, Business.working_days_bonus > 0)
            .values(working_days_bonus=Business.working_days_bonus - 1)
        )
        if result.rowcount == 0:
            result = await session.execute(
                update(Business)
                .where(Business.id == business.id, Business.working_days_remaining > 0)
                .values(working_days_remaining=Business.working_days_remaining - 1)
            )
            if result.rowcount == 0:
                raise HTTPException(status_code=402, detail="No working days remaining")
...
async def advance_roadmap_day(
    business: Business = Depends(get_business),
    session: AsyncSession = Depends(get_session),
):
    """Advance roadmap_day by 1 (used by test runners to simulate nightly progression)."""
    await session.execute(
        update(Business)
        .where(Business.id == business.id)
        .values(roadmap_day=Business.roadmap_day + 1)
    )
    await session.commit()
    await session.refresh(business)
    return {"roadmap_day": business.roadmap_day}
