...
Item 57: Preloads commonly needed session data (business, working days, plan)
...
class PreloadedSession:
    ...
    working_days_bonus: int = 0
    template: str | None = None
    loaded_at: float = field(default_factory=time.monotonic)

    @property
    def total_working_days(self) -> int:
        return self.working_days_remaining + self.working_days_bonus
        ...
        if business_slug:
            result = await session.execute(
                select(
                    ...
                    Business.name,
                    Business.plan,
                    Business.working_days_remaining,
                    Business.working_days_bonus,
                    Business.template,
            ...
            if row:
                ...
                preloaded.working_days_remaining = row[4]
                preloaded.working_days_bonus = row[5]
