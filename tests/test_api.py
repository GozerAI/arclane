...
async def test_trigger_cycle_no_working_days(auth_client):
    await auth_client.post("/api/businesses", json={
        "name": "No Credits",
        "description": "Testing no working days",
        "owner_email": "test@example.com",
    })
    # Exhaust remaining 2 preview working days (1 preview working day was used by the initial cycle)
    for i in range(2):
