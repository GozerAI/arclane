"""Billing edge cases — working day deduction, plan changes, concurrent ops."""
...
async def _create_business(factory, slug, plan="starter", working_days=5, bonus=0, email="test@test.com"):
...
async def test_cycle_deducts_bonus_working_day_first(client):
    """On-demand cycle deducts bonus working day before regular working days."""
    # Create a business with both bonus and regular working days
    await client.post("/api/businesses", json={
        "name": "Bonus First",
        "description": "Test bonus deduction order",
    })
    # The created business has default working days (5 regular, bonus gets decremented for initial cycle)
...
async def test_cycle_deducts_regular_when_no_bonus(client):
    """Cycle deducts regular working days when bonus is exhausted."""
    await client.post("/api/businesses", json={
        "name": "Regular Deduct",
        "description": "Test regular deduction",
    })
    # Trigger multiple cycles to exhaust bonus working days
    for _ in range(3):
...
# --- Insufficient working days blocking ---


async def test_cycle_blocked_when_no_working_days(db):
    """Cycle trigger returns 402 when working days are exhausted."""
    biz_id = await _create_business(db, "no-working-days", working_days=0, bonus=0)
    ...
    # Seed business with 0 working days in this new DB
    async with factory() as session:
        biz = Business(
            slug="zero-working-days",
            name="Zero Credits",
    ...
    with patch.object(cycles_module, "_run_cycle", mock_run):
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as c:
            ...
            # But we need a business owned by this user with 0 working days
            async with factory() as session:
                biz2 = Business(
                    slug="nocred-biz",
                    name="NoCred Biz",
            ...
            # Create business owned by this user with 0 working days
            async with factory() as session:
                biz3 = Business(
                    slug="nocred2-biz",
                    name="NoCred2 Biz",
            ...
            assert "no working days" in resp.json()["detail"].lower()
...
async def test_monthly_reset_restores_starter_working_days(db):
    """Starter subscriptions reset to the included monthly working days."""
    from arclane.engine.scheduler import _monthly_working_day_reset

    biz_id = await _create_business(db, "starter-reset", plan="starter", working_days=1)
    ...
    async with db() as session:
        ...
        assert biz.working_days_remaining == 10


async def test_monthly_reset_restores_pro_working_days(db):
    """Monthly reset gives pro plan 20 working days."""
    from arclane.engine.scheduler import _monthly_working_day_reset

    biz_id = await _create_business(db, "pro-reset", plan="pro", working_days=3)
    ...
    async with db() as session:
        biz = await session.get(Business, biz_id)
        assert biz.working_days_remaining == 20


async def test_monthly_reset_restores_scale_working_days(db):
    """Monthly reset gives scale plan 150 working days."""
    from arclane.engine.scheduler import _monthly_working_day_reset

    biz_id = await _create_business(db, "scale-reset", plan="scale", working_days=50)
...
async def test_webhook_upgrades_plan_to_pro(client):
    """Subscription webhook upgrades plan and working days."""
    from arclane.core.config import settings
...
async def test_webhook_upgrades_plan_to_growth(client):
    """Subscription webhook upgrades to growth plan with correct working days."""
    from arclane.core.config import settings
    orig_token = settings.zuul_service_token
    settings.zuul_service_token = "test-secret"
    try:
...
async def test_webhook_cancels_subscription(client):
    """Cancellation webhook sets plan to cancelled and zeroes working days."""
    from arclane.core.config import settings
    orig_token = settings.zuul_service_token
    settings.zuul_service_token = "test-secret"
    try:
...
async def test_cancelled_plan_gets_no_working_days_on_reset(db):
    """Cancelled businesses get no working days on monthly reset."""
    from arclane.engine.scheduler import _monthly_working_day_reset

    biz_id = await _create_business(db, "cancelled-biz", plan="cancelled", working_days=0)
...
# --- Bonus working day on first business creation ---


async def test_initial_cycle_uses_bonus_working_day(client):
    """Creating a business consumes one preview working day for the initial cycle."""
    ...
    assert resp.status_code == 201
    data = resp.json()
    assert data["working_days_remaining"] == 2


async def test_default_bonus_working_days_are_ten(db):
    """New businesses default to no bonus working days."""
    async with db() as session:
        biz = Business(
            slug="bonus-default",
            name="Bonus Default",
        ...
        assert biz.working_days_bonus == 0


# --- Credit refund on failed cycle ---


async def test_nightly_cycle_skips_zero_working_day_business(db):
    """Nightly cycle skips businesses with no working days."""
    from arclane.engine.scheduler import _nightly_cycle

    await _create_business(db, "broke-biz", working_days=0, bonus=0)
...
async def test_nightly_uses_bonus_before_regular(db):
    """Nightly cycle deducts from bonus working days before regular."""
    from arclane.engine.scheduler import _nightly_cycle

    biz_id = await _create_business(db, "bonus-prio", working_days=5, bonus=3)
    ...
    async with db() as session:
        ...
        assert biz.working_days_bonus == 2
        assert biz.working_days_remaining == 5


async def test_nightly_uses_purchased_add_on_capacity_before_balance(db):
    """Queued add-ons use their own included nights before touching business working days."""
    from arclane.engine.scheduler import _nightly_cycle
...
async def test_billing_status_returns_plan_info(client):
    """Billing status endpoint returns plan and working day info."""
    await client.post("/api/businesses", json={
        "name": "Status Check",
    ...
    assert "working_days_remaining" in data
    assert data["company_limit"] == 1
    assert data["trial_days"] == 3
    assert data["can_start_paid_trial"] is True


async def test_billing_status_shows_combined_working_days(client):
    """Billing status reflects remaining preview working days."""
    await client.post("/api/businesses", json={
        "name": "Combined Credits",
        "description": "Testing combined working days",
    })
...
async def test_working_day_purchase_webhook_adds_bonus_working_days(client):
    """Working day purchase webhook adds top-up working days to the business."""
    from arclane.core.config import settings
    ...
    try:
        await client.post("/api/businesses", json={
            "name": "Burst Credits",
            "description": "Testing purchased working days",
        })
        resp = await client.post(
        ...
        assert status.json()["working_days_remaining"] == 7
    finally:
        settings.zuul_service_token = orig_token
...
async def test_webhook_renewal_restores_working_days(client):
    """Renewal webhook restores working days for the plan."""
    from arclane.core.config import settings
    orig_token = settings.zuul_service_token
    settings.zuul_service_token = "test-secret"
    try:
        ...
        # First deplete working days by triggering cycles
...
async def test_checkout_invalid_day_pack_returns_400(client):
    """Checkout with invalid day pack returns 400."""
    await client.post("/api/businesses", json={
        "name": "Bad Pack",
        "description": "Testing invalid day pack",
    })
    resp = await client.post(
        "/api/businesses/bad-pack/billing/checkout",
        json={"day_pack": "nonexistent-pack"},
    )
    assert resp.status_code == 400
...
async def test_multiple_resets_are_idempotent(db):
    ...
    biz_id = await _create_business(db, "idempotent-reset", plan="pro", working_days=5)
