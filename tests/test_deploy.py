...
async def test_deploy_simulated_without_docker(db, tmp_path):
    ...
    manifest = json.loads((tmp_path / "workspaces" / "sim-test" / "arclane-workspace.json").read_text())
    assert manifest["slug"] == "sim-test"
    assert manifest["storage_mode"] == "workspace_copy"


async def test_wait_for_tenant_surface_returns_false_for_closed_port():
    from arclane.provisioning.deploy import wait_for_tenant_surface

    ready = await wait_for_tenant_surface(65500, attempts=1, interval_s=0)
