...
from arclane.notifications import (
    send_working_days_low_email,
    send_cycle_complete_email,
    send_mailbox_ready_email,
...
@pytest.mark.asyncio
@patch("arclane.provisioning.email.send_email", new_callable=AsyncMock)
async def test_working_days_low_email_calls_send_email(mock_send):
    await send_working_days_low_email("My Biz", "owner@test.com", 2)
...
@pytest.mark.asyncio
@patch("arclane.provisioning.email.send_email", new_callable=AsyncMock)
async def test_working_days_low_email_does_not_raise_on_error(mock_send):
    mock_send.side_effect = RuntimeError("Resend is down")
    await send_working_days_low_email("My Biz", "owner@test.com", 1)
...
@pytest.mark.asyncio
@patch("arclane.provisioning.email.send_email", new_callable=AsyncMock)
async def test_mailbox_ready_email_calls_send_email(mock_send):
    await send_mailbox_ready_email("My Biz", "owner@test.com", "my-biz", "my-biz@arclane.cloud")

    mock_send.assert_called_once()
    args = mock_send.call_args[0]
    assert args[0] == "arclane"
    assert args[1] == "owner@test.com"
    assert "Business address configured" in args[2]
    assert "my-biz@arclane.cloud" in args[3]


@pytest.mark.asyncio
@patch("arclane.provisioning.email.send_email", new_callable=AsyncMock)
async def test_mailbox_ready_email_does_not_raise_on_error(mock_send):
    mock_send.side_effect = RuntimeError("Resend is down")
    await send_mailbox_ready_email("My Biz", "owner@test.com", "my-biz", "my-biz@arclane.cloud")
