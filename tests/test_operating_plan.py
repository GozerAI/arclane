...
def test_operating_plan_includes_tasks_recommendations_and_storage():
    ...
    assert plan["credit_model"]["definition"] == "One credit equals one nightly execution cycle for one business."
    assert len(plan["agent_tasks"]) == 5
    assert plan["agent_tasks"][1]["duration_days"] == 2
    assert plan["agent_tasks"][1]["days_remaining"] == 2
...
def test_enqueue_add_on_inserts_queue_cutting_work():
    ...
    assert updated["agent_tasks"][0]["credits_required"] == 3
