...
class TestCDNCacheHeaders:
    ...
    def test_static_cache_duration_js(self):
        ...
        assert get_cache_duration("/static/app.js") == 86400 * 30
...
class TestSessionPreloader:
    ...
    def test_preloaded_session_defaults(self):
        ...
        assert ps.total_credits == 0

    def test_preloaded_session_with_data(self):
        from arclane.performance.session_preload import PreloadedSession
        ps = PreloadedSession(
            business_id=1, credits_remaining=5, credits_bonus=10,
        )
        assert ps.is_loaded is True
        assert ps.total_credits == 15
...
class TestBusinessConfigCache:
    ...
    def test_put_and_get(self):
        ...
        biz = MagicMock(
            ...
            credits_remaining=5, credits_bonus=10,
        )
        bc.put(biz)
        result = bc.get("test")
        assert result is not None
    ...
    def test_get_expired(self):
        ...
        biz = MagicMock(
            ...
            credits_remaining=0, credits_bonus=0,
        )
        bc.put(biz)
