"""Tests for credit-based metered billing, pay-per-cycle, usage caps,
dynamic pricing, credit rollover, gifting, business-size pricing,
ROI calculator, discounts, marketplace, auto-upgrade, referral bonuses,
variable pricing, credit expiration, spending caps, prepaid packages,
...
def starter_account(engine):
    acct = CreditAccount(business_id=1, plan="starter", credits_remaining=5)
    return engine.register_account(acct)


@pytest.fixture
def pro_account(engine):
    acct = CreditAccount(business_id=2, plan="pro", credits_remaining=20)
    return engine.register_account(acct)
...
class TestMeteredBilling:
    def test_consume_credit_basic(self, engine, starter_account):
        txn = engine.consume_credit(1)
        assert txn.delta == -1
        assert txn.balance_after == 4
        assert starter_account.credits_remaining == 4

    def test_consume_multiple_credits(self, engine, starter_account):
        txn = engine.consume_credit(1, amount=3)
        assert txn.delta == -3
        assert starter_account.credits_remaining == 2

    def test_consume_insufficient_credits(self, engine, starter_account):
        with pytest.raises(ValueError, match="Insufficient credits"):
            engine.consume_credit(1, amount=10)

    def test_metered_billing_mode_tracks_cost(self, engine):
        acct = CreditAccount(
            business_id=10, plan="pro", credits_remaining=20,
            billing_mode=BillingMode.METERED.value)
        engine.register_account(acct)
        engine.consume_credit(10, amount=2)
        assert acct.metered_usage_cents > 0

    def test_consume_with_feature(self, engine, starter_account):
        txn = engine.consume_credit(1, feature="content_generation")
        assert txn.feature == "content_generation"

    def test_consume_updates_total_spent(self, engine, starter_account):
        engine.consume_credit(1, amount=2)
...
class TestUsageCaps:
    ...
    def test_soft_limit_warning(self, engine):
        acct = CreditAccount(business_id=3, plan="starter", credits_remaining=1)
        engine.register_account(acct)
    ...
    def test_hard_limit_warning(self, engine):
        acct = CreditAccount(business_id=4, plan="starter", credits_remaining=0)
        engine.register_account(acct)
    ...
    def test_custom_soft_limit(self, engine):
        acct = CreditAccount(
            business_id=5, plan="starter", credits_remaining=2,
            soft_limit_pct=50.0)
        engine.register_account(acct)
    ...
    def test_hard_cap_prevents_consumption(self, engine):
        acct = CreditAccount(
            business_id=6, plan="starter", credits_remaining=2, hard_cap=2)
        engine.register_account(acct)
        engine.consume_credit(6, amount=1)
        engine.consume_credit(6, amount=1)
        with pytest.raises(ValueError, match="Hard usage cap"):
            engine.consume_credit(6, amount=1)


# --- 262: Dynamic credit pricing based on demand ---
...
class TestCreditRollover:
    def test_rollover_annual_pro(self, engine):
        acct = CreditAccount(
            business_id=7, plan="pro", credits_remaining=10,
            annual_billing=True)
        engine.register_account(acct)
        rollover = engine.process_rollover(7)
        assert rollover > 0
        assert acct.rollover_credits == rollover
    ...
    def test_no_rollover_starter_annual(self, engine):
        acct = CreditAccount(
            business_id=8, plan="starter", credits_remaining=3,
            annual_billing=True)
        engine.register_account(acct)
    ...
    def test_rollover_capped_at_plan_pct(self, engine):
        acct = CreditAccount(
            business_id=9, plan="growth", credits_remaining=50,
            annual_billing=True)
        engine.register_account(acct)
    ...
    def test_rollover_creates_transaction(self, engine):
        acct = CreditAccount(
            business_id=11, plan="scale", credits_remaining=100,
            annual_billing=True)
        engine.register_account(acct)
...
class TestCreditGifting:
    def test_gift_credits(self, engine, starter_account, pro_account):
        txn_s, txn_r = engine.gift_credits(2, 1, 5)
        assert txn_s.delta == -5
        assert txn_r.delta == 5
        assert starter_account.credits_remaining == 10
        assert pro_account.credits_remaining == 15

    def test_gift_insufficient_credits(self, engine, starter_account, pro_account):
        with pytest.raises(ValueError, match="Insufficient"):
            engine.gift_credits(1, 2, 100)

    def test_gift_zero_raises(self, engine, starter_account, pro_account):
        with pytest.raises(ValueError, match="positive"):
            engine.gift_credits(1, 2, 0)

    def test_gift_tracks_gifted_amounts(self, engine, starter_account, pro_account):
        engine.gift_credits(2, 1, 3)
        assert pro_account.credits_gifted_out == 3
        assert starter_account.credits_gifted_in == 3
...
class TestBusinessSizePricing:
    def test_solo_discount(self, engine):
        acct = CreditAccount(
            business_id=20, plan="pro", credits_remaining=20, employee_count=1)
        engine.register_account(acct)
    ...
    def test_enterprise_size_premium(self, engine):
        acct = CreditAccount(
            business_id=21, plan="pro", credits_remaining=20,
            employee_count=5000)
        engine.register_account(acct)
    ...
    def test_small_business_neutral(self, engine):
        acct = CreditAccount(
            business_id=22, plan="starter", credits_remaining=5,
            employee_count=25)
        engine.register_account(acct)
...
class TestDiscountPrograms:
    ...
    def test_ineligible_plan_raises(self, engine):
        acct = CreditAccount(business_id=30, plan="enterprise", credits_remaining=999)
        engine.register_account(acct)
...
# --- 280: Credit marketplace for unused credits ---

class TestCreditMarketplace:
    def test_create_listing(self, engine, pro_account):
        listing = engine.create_marketplace_listing(2, credits=5, price_cents_per_credit=500)
        assert listing.status == "active"
        assert listing.credits == 5
        assert pro_account.marketplace_listed_credits == 5

    def test_listing_exceeds_available(self, engine, starter_account):
        with pytest.raises(ValueError, match="available"):
            engine.create_marketplace_listing(1, credits=100, price_cents_per_credit=500)

    def test_purchase_listing(self, engine, starter_account, pro_account):
        listing = engine.create_marketplace_listing(2, credits=5, price_cents_per_credit=400)
        ...
        assert starter_account.credits_remaining == 10  # 5 + 5

    def test_purchase_inactive_listing(self, engine, starter_account, pro_account):
        listing = engine.create_marketplace_listing(2, credits=3, price_cents_per_credit=400)
        listing.status = "expired"
        with pytest.raises(ValueError, match="not active"):
            engine.purchase_marketplace_listing(listing, 1)

    def test_listing_zero_credits_raises(self, engine, pro_account):
        with pytest.raises(ValueError, match="at least 1"):
            engine.create_marketplace_listing(2, credits=0, price_cents_per_credit=500)
...
class TestAutoUpgrade:
    def test_suggest_upgrade_high_usage(self, engine):
        acct = CreditAccount(
            business_id=40, plan="starter", credits_remaining=0)
        engine.register_account(acct)
    ...
    def test_no_upgrade_top_tier(self, engine):
        acct = CreditAccount(
            business_id=41, plan="enterprise", credits_remaining=0)
        engine.register_account(acct)
    ...
    def test_suggestion_confidence_range(self, engine):
        acct = CreditAccount(
            business_id=42, plan="starter", credits_remaining=1,
...
class TestReferralBonus:
    def test_referral_bonus(self, engine, starter_account, pro_account):
        txn_ref, txn_new = engine.process_referral(1, 2, bonus_credits=5)
        assert txn_ref.delta == 5
        assert txn_new.delta == 5
        assert starter_account.credits_remaining == 10
        assert pro_account.credits_remaining == 25
...
class TestVariablePricing:
    def test_complex_feature_costs_more(self, engine):
        acct = CreditAccount(
            business_id=50, plan="pro", credits_remaining=20)
        engine.register_account(acct)
        # white_label has complexity 3.0
        txn = engine.consume_credit(50, amount=1, feature="white_label")
        assert abs(txn.delta) >= 3  # 1 * 3.0 rounded

    def test_simple_feature_base_cost(self, engine):
        acct = CreditAccount(
            business_id=51, plan="pro", credits_remaining=20)
        engine.register_account(acct)
        txn = engine.consume_credit(51, amount=1, feature="social_scheduling")
        assert abs(txn.delta) == 1  # 1 * 0.5 rounds to 1 (min 1)

    def test_no_feature_base_cost(self, engine, starter_account):
        txn = engine.consume_credit(1, amount=1)
...
class TestCreditExpiration:
    def test_set_expiration(self, engine, starter_account):
        exp = engine.set_credit_expiration(1, credits=3, days=30)
        assert exp.credits == 3
        assert not exp.expired

    def test_expiration_triggers_7d_notification(self, engine, starter_account):
        exp = engine.set_credit_expiration(1, credits=3, days=5)
    ...
    def test_expiration_triggers_1d_notification(self, engine, starter_account):
        exp = engine.set_credit_expiration(1, credits=3, days=1)
        now = datetime.now(timezone.utc) + timedelta(hours=12)
        results = engine.check_expirations(now)
        assert any(r.notified_1d for r in results)

    def test_expired_credits_deducted(self, engine, starter_account):
        engine.set_credit_expiration(1, credits=3, days=1)
        now = datetime.now(timezone.utc) + timedelta(days=2)
        results = engine.check_expirations(now)
        assert any(r.expired for r in results)
        assert starter_account.credits_remaining == 2  # 5 - 3

    def test_already_expired_not_reprocessed(self, engine, starter_account):
        engine.set_credit_expiration(1, credits=2, days=1)
        now = datetime.now(timezone.utc) + timedelta(days=2)
...
class TestSpendingCaps:
    ...
    def test_spending_cap_enforced(self, engine):
        acct = CreditAccount(
            business_id=60, plan="pro", credits_remaining=100,
            spending_cap_cents=1000)
        engine.register_account(acct)
        # consume credits until cap hit
        with pytest.raises(ValueError, match="Spending cap"):
            for _ in range(20):
                engine.consume_credit(60)

    def test_spending_remaining(self, engine, starter_account):
        engine.set_spending_cap(1, cap_cents=10000)
        engine.consume_credit(1)
...
# --- 451: Prepaid credit packages with bonus credits ---
...
class TestPrepaidPackages:
    ...
    def test_package_updates_bonus(self, engine, starter_account):
        ...
        assert starter_account.credits_bonus == 3
...
# --- 460: Credit purchase history and projections ---
...
class TestHistoryAndProjections:
    ...
    def test_projection_with_usage(self, engine, starter_account):
        engine.consume_credit(1)
        engine.consume_credit(1)
        proj = engine.project_credit_usage(1)
        assert proj["current_credits"] == 3
    ...
    def test_history_limit(self, engine, starter_account):
        for _ in range(5):
            engine.consume_credit(1)
            starter_account.credits_remaining += 2
...
class TestUsageReporting:
    def test_usage_by_feature(self, engine):
        acct = CreditAccount(
            business_id=70, plan="pro", credits_remaining=50)
        engine.register_account(acct)
        engine.consume_credit(70, feature="content_generation")
        engine.consume_credit(70, feature="content_generation")
        engine.consume_credit(70, feature="seo_optimization")
