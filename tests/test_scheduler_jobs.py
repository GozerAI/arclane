...
async def test_nightly_runs_for_active_businesses(db):
    """Nightly cycle runs for businesses with working days."""
    from arclane.engine.scheduler import _nightly_cycle

    await _create_business(db, "active-biz", working_days=5, bonus=0)
...
async def test_nightly_skips_zero_working_days(db):
...
async def test_nightly_skips_cancelled_plan(db):
    ...
    await _create_business(db, "cancelled-biz", plan="cancelled", working_days=5, bonus=0)
...
async def test_nightly_deducts_one_working_day(db):
    """Nightly cycle deducts exactly one working day per run."""
    from arclane.engine.scheduler import _nightly_cycle

    biz_id = await _create_business(db, "deduct-one", working_days=5, bonus=0)
    ...
    async with db() as session:
        ...
        assert biz.working_days_remaining == 4
...
async def test_nightly_creates_cycle_record(db):
    ...
    biz_id = await _create_business(db, "cycle-rec", working_days=5, bonus=0)
...
async def test_nightly_runs_multiple_businesses(db):
    ...
    await _create_business(db, "multi1", working_days=3, bonus=0)
    await _create_business(db, "multi2", working_days=5, bonus=0)
    await _create_business(db, "multi3", working_days=1, bonus=0)
...
async def test_nightly_deducts_bonus_before_regular(db):
    """Nightly deducts bonus working days before regular working days."""
    from arclane.engine.scheduler import _nightly_cycle

    biz_id = await _create_business(db, "bonus-first", working_days=5, bonus=2)
    ...
    async with db() as session:
        biz = await session.get(Business, biz_id)
        assert biz.working_days_bonus == 1  # 2 - 1
        assert biz.working_days_remaining == 5  # untouched
...
async def test_nightly_handles_orchestrator_exception(db):
    ...
    await _create_business(db, "fail-biz", working_days=5, bonus=0)
    await _create_business(db, "ok-biz", working_days=5, bonus=0)
...
# --- Monthly working day reset ---


async def test_monthly_reset_all_plans(db):
    """Monthly reset restores correct working days for each plan."""
    from arclane.engine.scheduler import _monthly_working_day_reset, PLAN_WORKING_DAYS

    ids = {}
    for plan, expected in PLAN_WORKING_DAYS.items():
        ids[plan] = await _create_business(db, f"plan-{plan}", plan=plan, working_days=1)
    ...
    async with db() as session:
        for plan, expected in PLAN_WORKING_DAYS.items():
            ...
            assert biz.working_days_remaining == expected, f"{plan} should get {expected} working days"
...
async def test_monthly_reset_skips_unknown_plan(db):
    ...
    biz_id = await _create_business(db, "custom-plan", plan="custom", working_days=99)
    ...
    async with db() as session:
        biz = await session.get(Business, biz_id)
        assert biz.working_days_remaining == 99  # unchanged


async def test_monthly_reset_skips_cancelled(db):
    """Cancelled businesses get no working days on reset."""
    from arclane.engine.scheduler import _monthly_working_day_reset

    biz_id = await _create_business(db, "canc", plan="cancelled", working_days=0)
...
# --- PLAN_WORKING_DAYS mapping ---


def test_plan_working_days_mapping():
    """PLAN_WORKING_DAYS has correct values for all plans."""
    from arclane.engine.scheduler import PLAN_WORKING_DAYS

    assert PLAN_WORKING_DAYS["starter"] == 10
    assert PLAN_WORKING_DAYS["pro"] == 20
    assert PLAN_WORKING_DAYS["growth"] == 75
    assert PLAN_WORKING_DAYS["scale"] == 150
    assert "cancelled" not in PLAN_WORKING_DAYS
