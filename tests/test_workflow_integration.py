"""Cross-repo integration tests: workflow → task conversion → engine payload.

Tests the full pipeline from .ail workflow files through to the task format
expected by engine's POST /api/v1/arclane/cycle endpoint.
"""
...
# engine ArclaneTaskItem expected fields
REQUIRED_TASK_FIELDS = {"area", "action", "description"}

# engine _ARCLANE_TASK_MAP keys (valid areas the bridge recognizes)
VALID_AREAS = {
...
class TestWorkflowTaskFormat:
    """Verify that workflow-generated tasks match engine's expected format."""
    ...
    def test_areas_are_valid(self, workflow_tasks):
        for task in workflow_tasks:
            assert task["area"] in VALID_AREAS, (
                f"Unknown area '{task['area']}' — engine will default to general"
            )
...
class TestOrchestratorWorkflowIntegration:
    ...
    def test_all_templates_produce_valid_payloads(self):
        """Simulate what would be sent to engine for each template."""
        orch = ArclaneOrchestrator()
